...
export default class MultiSwitch extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            thresholdDistance: width - 8 - width / 3
        };
            ...
            onPanResponderRelease: (evt, gestureState) => {
                ...
                if (gestureState.dx > 0) {
                    ...
                    if (finalValue >= 78 && finalValue <= 175) {
                        ...
                        this.notStartedSelected();
                    } else if (finalValue >= 175 && finalValue <= 272) {
                        this.completeSelected();
                    } else {
                        console.log("got finishedSelected");
    ...
    inProgressSelected = () => {
        Animated.timing(this.state.position, {
            toValue: this.state.mainWidth / 3 - this.state.switcherWidth / 3,
            duration: this.state.duration
        }).start();
        setTimeout(() => {
            this.setState({
                posValue:
                    this.state.mainWidth / 3 - this.state.switcherWidth / 3,
                selectedPosition: 1
            });
        }, 100);
    ...
    completeSelected = () => {
        ...
        Animated.timing(this.state.position, {
            toValue:
                Platform.OS === 'ios'
                    ? this.state.mainWidth/1.5 - this.state.switcherWidth/1.5
                    : this.state.mainWidth/1.5 - this.state.switcherWidth/1.5,
            duration: this.state.duration
        }).start();
        setTimeout(() => {
    ...
    FinishSelected = () => {
        Animated.timing(this.state.position, {
            toValue:
                Platform.OS === 'ios'
                    ? this.state.mainWidth - this.state.switcherWidth
                    : this.state.mainWidth - this.state.switcherWidth +10,
            duration: this.state.duration
        }).start();
        setTimeout(() => {
            this.setState({
        ...
        if (this.state.isComponentReady) this.props.onStatusChanged('Finished');
    };
    ...
    render() {
        return (
            ...
            <View style={styles.container}>
                ...
                <Button type="Finished" onPress={this.FinishSelected} />
                <Animated.View
